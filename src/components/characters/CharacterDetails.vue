  ...
  <v-card class="CharacterDetails" variant="outlined">
    <div style="overflow-y: auto; max-height: calc(100vh - 95px);">
      <div class="CharacterDetails-title">
        ...
        <v-card-subtitle class="text-caption pb-1">{{ getDescription() }} - {{ characterData.category }}</v-card-subtitle>
      </div>

      <div class="text-body-2 text-high-emphasis pa-4">
        ...
        <p class="mb-1"><span class="font-weight-bold">Speed</span> {{ getSpeedString() }}</p>
        <p class="mb-1"><span class="font-weight-bold">Senses</span> {{ characterData.senses || "&mdash;" }}</p>
        <p class="mb-1"><span class="font-weight-bold">Languages</span> {{ characterData.languages || "&mdash;" }}</p>
        <p class="mb-1"><span class="font-weight-bold">Challenge Rating</span> {{ characterData.challenge_rating || "&mdash;" }}</p>
        <p class="mb-1"><span class="font-weight-bold">Damage Immunities</span> {{ characterData.damage_immunities || "&mdash;" }}</p>
        <p class="mb-1"><span class="font-weight-bold">Damage Resistances</span> {{ characterData.damage_resistances || "&mdash;" }}</p>
        <p class="mb-1"><span class="font-weight-bold">Damage Vulnerabilities</span> {{ characterData.damage_vulnerabilities || "&mdash;" }}</p>
      </div>
      ...
      <div class="pa-4">
        ...
        <template v-if="characterData.legendary_actions.length > 0">
          ...
          <v-divider class="mb-2"/>
          <p>The {{ characterData.name }} can take 3 legendary actions, choosing from the options below. Only one legendary action option can be used at a time and only at the end of another creature's turn. The {{ characterData.name }} regains spent legendary actions at the start of its turn.</p>
